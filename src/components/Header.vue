						...
						<div class="fr-header__tools">
							<div class="fr-header__tools-links">
								<ul class="fr-links-group">
									...
									<li>
										<router-link class="fr-link fr-fi-add-circle-line" to="/me">Mon espace</router-link>
									</li>
								</ul>
							</div>
				...
				<div class="fr-container">
					...
					<nav class="fr-nav" id="navigation-791" role="navigation" aria-label="Menu principal">
						<ul class="fr-nav__list">
							...
							<li class="fr-nav__item">
								<router-link to="/app" class="fr-nav__link" aria-expanded="false">App</router-link>
							</li>
							<li class="fr-nav__item">
								<div to="#" class="cm-click fr-nav__link" @click="logout">Logout</div>
							</li>
						</ul>
					</nav>
				</div>
	...
	export default {
		...
		methods:{
			logout(e){
				...
				if(!token){
					return this.dialog = this.$dialogs.error
				}
				...
				axios
					...
					.then(response => {
						this.dialog = this.$dialogs.logout_success
						localStorage.removeItem('token')
					})
					.catch(e =>{
						this.dialog = this.$dialogs.error
					})
			}
		}
	}
